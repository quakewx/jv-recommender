...
(function () {
  ...
  const CONFIG = {
    ...
    gradeMagnet: true,       // 磁链分级展示：为推荐影片选出“最佳磁链”（中字/HD/无码破解等标签）并支持一键复制
    actorPages: 3,          // “演员作品”源抓取页数（每页 pageSize 部，3 页≈120 部候选）
  ...
  // 每项磁链字段：name / hash / size(MiB，兆字节) / cnsub(中字bool) / hd(bool) / files_count / created_at / pikpak_url
  // 缓存值语义：count 数字=磁链条数，null=未知（请求失败/超时/站点不支持），绝不可当成 0 过滤掉。
  const magnetCache = new Map(); // id -> { count, best }

  // 磁链分级：参考 JAVDB_AutoSpider 优先级（中字 > 无码破解 > 无码 > 无字幕），结合 API 现成布尔字段
  function gradeMagnet(m) {
    let score = 0; const tags = [];
    const name = (m.name || '').toLowerCase();
    if (m.cnsub) { score += 100; tags.push('中字'); }       // 中文字幕优先
    if (m.hd) { score += 40; tags.push('HD'); }
    if (/破解|crack|fc2ppv/i.test(name)) { score += 60; tags.push('破解'); }
    else if (/无码|uncensored|\buc\b/i.test(name)) { score += 50; tags.push('无码'); }
    if (/fc2/i.test(name)) { score += 20; tags.push('FC2'); }
    if (/4k|2160p/i.test(name)) { score += 15; tags.push('4K'); }
    const szMiB = m.size || 0;                              // API 返回的 size 是 MiB（兆字节），非字节
    const sz = szMiB * 1024 * 1024;                         // 换算成字节用于体积加权
    if (sz > 1024 * 1024 * 1024) score += 30;              // >1GB
    else if (sz > 300 * 1024 * 1024) score += 12;          // >300MB
    return { score, label: tags.join('·') || '磁链', magnetUrl: 'magnet:?xt=urn:btih:' + m.hash };
  }

  // 从磁链数组里挑“最佳”：分数最高，同分取体积更大（更新更可能高清）
  function pickBestMagnet(arr) {
    let best = null, bestScore = -1;
    for (const m of arr) {
      const g = gradeMagnet(m);
      if (best == null || g.score > bestScore || (g.score === bestScore && (m.size || 0) > (best.size || 0))) {
        bestScore = g.score;
        best = { name: m.name, size: m.size, created_at: m.created_at, cnsub: m.cnsub, hd: m.hd, magnetUrl: g.magnetUrl, label: g.label };
      }
    }
    return best;
  }

  async function fetchMagnetInfo(id) {
    if (!id) return { count: null, best: null };
    if (magnetCache.has(id)) return magnetCache.get(id);
    let result = { count: null, best: null };
    try {
      const d = await Promise.race([
        jdbApiGet('/api/v1/movies/' + id + '/magnets'),
        sleep(CONFIG.magnetTimeoutMs).then(() => { throw new Error('timeout'); }),
      ]);
      const arr = (d && d.magnets) ? d.magnets : [];
      if (Array.isArray(arr) && arr.length) {
        result.count = arr.length;
        result.best = pickBestMagnet(arr);
      } else {
        result.count = 0; // 空数组 = 确认无磁链
      }
    } catch (e) { result.count = null; }
    magnetCache.set(id, result);
    return result;
  }

  // 兼容旧调用：返回磁链条数（0=无, null=未知）
  async function fetchMagnetCount(id) {
    const r = await fetchMagnetInfo(id);
    return r.count;
  }
  ...
  const JDB_API = {
    ...
    // 候选节点：主站 + 多个 App API 镜像（借鉴 miyabi route.go 的 bootstrapHosts；实测 apidd.* 镜像延迟更低）
    hosts: ['https://javdb.com', 'https://jdforrepam.com', 'https://apidd.spthgb.com', 'https://apidd.czssdgz.com'],
  ...
  // ---- 主机选路（借鉴 miyabi route.go）----
  // 用带签名的 /api/v1/startup 逐候选节点探测，按延迟选最快的可用节点并持久化；
  // 遇 502/503/504/网络错误自动清除选中节点，下次请求重新探测。并发请求共享同一次探测（singleflight）。
  const JDB_HOST_KEY = 'jvr_jdb_host';
  let jdbSelectedHost = (typeof GM_getValue === 'function') ? (GM_getValue(JDB_HOST_KEY, '') || '') : '';
  let jdbSelecting = null;

  function jdbProbe(host) {
    const sig = jdbSign();
    const p = jdbPublicParams();
    const qs = Object.keys(p).map((k) => encodeURIComponent(k) + '=' + encodeURIComponent(p[k])).join('&');
    const url = host + '/api/v1/startup' + (qs ? '?' + qs : '');
    return new Promise((resolve, reject) => {
      const t0 = Date.now();
      GM_xmlhttpRequest({
        method: 'GET', url,
        ...
        onload: (r) => {
          if (r.status >= 200 && r.status < 300) resolve(Date.now() - t0);
  ...
  async function jdbSelectHost(force) {
    if (!force && jdbSelectedHost) return jdbSelectedHost;
    if (jdbSelecting) return jdbSelecting; // 并发共享一次探测
    jdbSelecting = (async () => {
      const candidates = JDB_API.hosts.slice();
      log('主机选路: 探测', candidates.length, '个候选节点…');
      const results = await Promise.all(candidates.map(async (h) => {
        try { const lat = await jdbProbe(h); return { h, lat, ok: true }; }
        catch (e) { return { h, lat: Infinity, ok: false, err: e.message }; }
      }));
      results.sort((a, b) => a.lat - b.lat);
      const best = results.find((r) => r.ok);
      if (best) {
        jdbSelectedHost = best.h;
        if (typeof GM_setValue === 'function') GM_setValue(JDB_HOST_KEY, best.h);
        log('主机选路: 选中', best.h, '(' + best.lat + 'ms)');
      } else {
        log('主机选路: 全部失败，退回首个候选');
        jdbSelectedHost = candidates[0];
      }
      return jdbSelectedHost;
    })();
    try { return await jdbSelecting; }
    finally { jdbSelecting = null; }
  }

  function jdbClearHost() {
    jdbSelectedHost = '';
    if (typeof GM_setValue === 'function') GM_setValue(JDB_HOST_KEY, '');
  }

  // GM_xhr GET JSON，返回 envelope.data（已解析对象）；选中节点优先，其余按序回退；
  // 5xx / 网络错误触发重选（下次请求生效）
  async function jdbApiGet(path, extraParams) {
    await jdbSelectHost(false);
    const params = Object.assign(jdbPublicParams(), extraParams || {});
    const qs = Object.keys(params).map((k) => encodeURIComponent(k) + '=' + encodeURIComponent(params[k])).join('&');
    const sig = jdbSign();
    // 选中节点排第一，其余作为回退（去重）
    const ordered = [jdbSelectedHost].concat(JDB_API.hosts.filter((h) => h !== jdbSelectedHost));
    function tryHost(i) {
      if (i >= ordered.length) { jdbClearHost(); return Promise.reject(new Error('全部 host 失败: ' + path)); }
      const url = ordered[i] + path + (qs ? '?' + qs : '');
      return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          method: 'GET', url,
          headers: { 'jdsignature': sig, 'user-agent': JDB_API.userAgent, 'accept-language': 'en' },
          onload: (r) => {
            if (r.status === 502 || r.status === 503 || r.status === 504) {
              log('API host 5xx:', ordered[i], '→ 触发重选'); jdbClearHost(); reject(new Error('HTTP ' + r.status)); return;
            }
          ...
          onerror: () => { log('API host 网络错误:', ordered[i], '→ 触发重选'); jdbClearHost(); reject(new Error('网络错误')); },
        });
      }).catch((e) => {
        log('API host 失败:', ordered[i], '→', e.message, (i + 1 < ordered.length ? '尝试下一个' : '已无备用'));
        return tryHost(i + 1);
      });
    }
    return tryHost(0);
  ...
  function jdbMovieFromApi(m, baseOffset, i) {
    ...
    return {
      ...
      source: 'api',
      _apiId: m.id || '',
      // 搜索/演员源列表项自带 magnets_count，零额外请求即可判断磁链有无（>0 有, 0 确认无, 缺省未知）
      magnets: (m.magnets_count != null) ? m.magnets_count : null,
    };
  }
  ...
  const adapters = {
    javdb: {
      ...
      sources: [
        ...
        // 演员作品：按演员名搜索（filter_by=actor），服务端按评分排序；演员名从面板独立输入框取，不并入关键字/分类
        { id: 'actor', label: '演员作品', mode: 'api', paged: true, pages: CONFIG.actorPages, limit: 40, apiPath: '/api/v2/search', apiParams: (kw) => ({ q: kw || '', filter_by: 'actor', movie_sort_by: 'score' }) },
        { id: 'current', label: '当前页面', mode: 'html' },
        // 本片相似推荐：取当前影片详情(/api/v4/movies/{id})里的 actor_movies(同演员) + relative_movies(相关)，
        // 在 javdb 影片详情页(/v/xxx)打开即用；也可在关键字框手动输入番号指定基准影片。
        { id: 'similar', label: '本片相似推荐', mode: 'api' },
      ],
      // 常见分类（按名称走搜索，近似分类筛选）
      ...
      async apiFetch(sourceId, keyword, opts) {
        opts = opts || {};
        if (sourceId === 'similar') return this.fetchSimilar(opts);
      ...
      // 本片相似推荐：基于当前影片详情里的「同演员 + 相关」数组，是真正的“猜你喜欢”
      async fetchSimilar(opts) {
        opts = opts || {};
        // 优先取当前详情页 /v/{id}；其次允许在关键字框手动输入番号
        let id = (location.pathname.match(/\/v\/([^/?#]+)/) || [])[1] || '';
        if (!id) {
          const kwEl = panel && panel.querySelector ? panel.querySelector('#jvr-kw') : null;
          const kw = kwEl ? kwEl.value.trim() : '';
          if (kw) {
            try {
              const data = await jdbApiGet('/api/v2/search', { q: kw, movie_sort_by: 'score', limit: 5 });
              const ms = (data && data.movies) || [];
              const exact = ms.find((m) => m.number && m.number.toUpperCase() === kw.toUpperCase()) || ms[0];
              if (exact) id = exact.id;
            } catch (e) { log('相似推荐: 关键字解析失败', e.message); }
          }
        }
        if (!id) {
          log('相似推荐: 未定位基准影片');
          throw new Error('本片相似推荐需在影片详情页(/v/xxx)打开，或在关键字框输入番号');
        }
        log('本片相似推荐: 拉取', id, '详情…');
        const d = await jdbApiGet('/api/v4/movies/' + id);
        const mv = (d && d.movie) ? d.movie : (d || {});
        if (!mv || !mv.id) throw new Error('未取到影片详情');
        const refs = [];
        const grab = (arr) => { if (Array.isArray(arr)) refs.push.apply(refs, arr); };
        grab(mv.actor_movies); grab(mv.relative_movies);
        grab(mv.ActorMovies); grab(mv.RelatedMovies);
        if (!refs.length) throw new Error('该影片无相似/同演员推荐数据');
        const seen = new Set(); const list = [];
        refs.forEach((r) => {
          const rid = r.id || (r.number || '');
          if (rid && !seen.has(rid)) { seen.add(rid); list.push(r); }
        });
        log('本片相似推荐: 候选', list.length, '部（同演员 + 相关）');
        let movies = list.map((r, i) => ({
          id: r.id || '',
          uid: r.number || '',
          title: (r.title || r.number || '').trim(),
          cover: r.thumb_url || r.cover_url || r.thumbnail || '',
          rating: null, year: null, position: i + 1,
          url: r.id ? origin() + '/v/' + r.id : '',
          source: 'similar', _apiId: r.id || '',
          magnets: (r.magnets_count != null) ? r.magnets_count : null,
        }));
        if (movies.length > 120) movies = movies.slice(0, 120);
        await this.fillScores(movies, true); // 放宽补分数量，让更多相似片拿到真实评分
        return movies;
      },
      ...
      async fillScores(movies, full) {
        ...
        await mapLimit(top, CONFIG.concurrency, async (m) => {
          ...
          // 磁链走独立端点校验；列表已带 magnets_count 则跳过（失败/超时为 null 仍补查，不会被当成“无磁链”）
          if (m.magnets == null) m.magnets = await fetchMagnetCount(m._apiId);
          done++;
      ...
      async enrichCandidates(movies, yearFilter) {
        ...
        await mapLimit(top, CONFIG.concurrency, async (m) => {
          ...
          // 磁链独立端点校验（网页 id 与 App API 内部 id 一致，可直接复用）；列表已带则跳过
          if (m.magnets == null) m.magnets = await fetchMagnetCount(m.id);
          done++;
  ...
  async function ensureMagnets(picked, pool, randomize) {
    ...
    // 磁链分级：先为全部候选附加“最佳磁链”（标签 + 链接），供卡片展示/复制
    if (CONFIG.gradeMagnet) {
      const toGrade = picked.filter((m) => (m._apiId || m.id) && !m.bestMagnet && m.magnets !== 0);
      if (toGrade.length) {
        await mapLimit(toGrade, CONFIG.concurrency, async (m) => {
          const info = await fetchMagnetInfo(m._apiId || m.id);
          if (info.best) m.bestMagnet = info.best;
          if (m.magnets == null) m.magnets = info.count;
        });
      }
    }
    ...
    // 递补新加入的影片也补上分级标签
    if (CONFIG.gradeMagnet) {
      const miss = ok.filter((m) => (m._apiId || m.id) && !m.bestMagnet);
      if (miss.length) {
        await mapLimit(miss, CONFIG.concurrency, async (m) => {
          const info = await fetchMagnetInfo(m._apiId || m.id);
          if (info.best) m.bestMagnet = info.best;
        });
      }
  ...
  function buildPanel() {
    GM_addStyle(`
      ...
      #jvr-panel .jvr-magnet { display:inline-block; margin-top:5px; margin-right:6px; padding:2px 8px; border-radius:4px;
        background:#143a2a; color:#5fe39a; font-size:11px; font-weight:700; text-decoration:none; cursor:pointer; }
      #jvr-panel .jvr-magnet:hover { background:#1d5239; }
    ...
    panel.innerHTML = `
      ...
      <div class="jvr-b" id="jvr-body">
        ...
        <label id="jvr-kw-label">关键字（数据源选“关键字搜索”时生效）</label>
        <input id="jvr-kw" placeholder="如 SSIS, 女教师, 4K..." />
        <label id="jvr-actor-label" style="display:none;">演员名（数据源选“演员作品”时生效，可填中文/日文/罗马音）</label>
        <input id="jvr-actor" placeholder="如 田中レモン / 波多野结衣 / 三上悠亜" style="display:none;" />
    ...
    // 数据源切换时显示/隐藏对应输入区（演员名框、关键字框提示）
    const actorLabel = panel.querySelector('#jvr-actor-label');
    const actorInput = panel.querySelector('#jvr-actor');
    const kwLabel = panel.querySelector('#jvr-kw-label');
    const kwInput = panel.querySelector('#jvr-kw');
    const syncActorVisibility = () => {
      const isActor = sel.value === 'actor';
      actorLabel.style.display = isActor ? 'block' : 'none';
      actorInput.style.display = isActor ? 'block' : 'none';
      // 相似推荐：关键字框改作「手动输入番号（可选）」
      if (sel.value === 'similar') {
        if (kwLabel) kwLabel.textContent = '基准番号（可选，留空则用当前详情页影片）';
        if (kwInput) kwInput.placeholder = '如 SSIS-001 / IPX-001…';
      } else {
        if (kwLabel) kwLabel.textContent = '关键字（数据源选“关键字搜索”时生效）';
        if (kwInput) kwInput.placeholder = '如 SSIS, 女教师, 4K...';
      }
    };
    sel.onchange = syncActorVisibility;
    ...
    if (last) {
      ...
      if (panel.querySelector('#jvr-actor')) panel.querySelector('#jvr-actor').value = last.actor || '';
      syncActorVisibility();
  ...
  async function runRecommend(randomize) {
    ...
    const actorName = (panel.querySelector('#jvr-actor') ? panel.querySelector('#jvr-actor').value : '').trim();
    const cat = panel.querySelector('#jvr-cat').value;
    const yearFilter = parseInt(panel.querySelector('#jvr-year').value, 10) || 0;

    // 演员作品源：用独立演员输入框，不并入关键字/分类；其余源忽略该框
    if (sourceId === 'actor') {
      if (!actorName) {
        panel.querySelector('#jvr-results').innerHTML = '<div class="jvr-empty">“演员作品”源需输入演员名；想按关键字搜请改选“关键字搜索”</div>';
        log('演员源：缺少演员名，已终止');
        return;
      }
      keyword = actorName;
    ...
    // （演员源不走此分支，保持独立的 actor 搜索 filter_by=actor）
    const FIXED_LIST = ['hot_daily', 'hot_weekly', 'hot_monthly', 'rankings_movies', 'classic'];
    if (sourceId !== 'actor' && (cat || keyword)) {
      keyword = ((keyword || '') + ' ' + (cat || '')).trim();
      if (FIXED_LIST.indexOf(sourceId) >= 0) sourceId = 'search';
    }
    GM_setValue('jvr_last', { source: sourceId, cat, kw: keyword, actor: actorName, year: yearFilter, hide: CONFIG.bayes.hideLowScore, hideRec: CONFIG.hideRecommended });
  ...
  function render(list, mode) {
    ...
    list.forEach((m, i) => {
      ...
      // 磁链分级标签：中字/HD/无码破解等（参考 JAVDB_AutoSpider 优先级），点击复制磁链
      const magTag = (m.bestMagnet)
        ? `<a class="jvr-magnet" data-magnet="${m.bestMagnet.magnetUrl}" href="${m.bestMagnet.magnetUrl}" title="点击复制磁链">🧲 ${m.bestMagnet.label || '磁链'}</a>`
        : '';
      item.innerHTML = `
        ...
        <div class="jvr-meta">
          ...
          ${recTag}${magTag}<span class="jvr-score" style="background:${cA};color:${c}">${badge}综合 ${(m.composite * 100).toFixed(1)}</span>
        </div>`;
      el.appendChild(item);
      const magBtn = item.querySelector('.jvr-magnet');
      if (magBtn) magBtn.onclick = (e) => {
        e.preventDefault();
        const txt = magBtn.getAttribute('data-magnet');
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(txt).then(() => log('已复制磁链: ' + (m.uid || m.title))).catch(() => { location.href = txt; });
